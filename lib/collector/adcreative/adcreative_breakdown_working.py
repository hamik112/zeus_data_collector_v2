...
Created Time: 2017-06-21 14:34:08
"""
...
class AdCreativeBreakdownWorking(AdCreativeBase):
    def __init__(self, collector, account_queue, adcreative_path, access_token, **kwargs):
        super(AdCreativeBreakdownWorking, self).__init__(collector, account_queue, adcreative_path, access_token, **kwargs)
        self.logger			= logging.getLogger('zeus_ads_breakdown')
        self.wflogger		= logging.getLogger('zeus_ads_breakdown_error')
    ...
    获取Ad Insights 信息
    """
    def _get_ad_insights(self, ad_id, breakdown_attribute, start_dt, stop_dt):
        try:
            ad_insights = self.fb_api.get_ad_insights(ad_id, breakdown_attribute, start_dt, stop_dt)
            if ad_insights:
                self.logger.info('[%s] [%s] Info, ad_id:%s, breakdown_attribute:%s, start_dt:%s, stop_dt:%s, ad_insights:%s' % \
                    (self.class_name, '_get_ad_insights', ad_id, breakdown_attribute, start_dt, stop_dt, ad_insights))
                return ad_insights
            return {}
        except Exception, why:
            self.wflogger.exception('[%s] [%s] Exception, ad_id:%s, breakdown_attribute:%s, start_dt:%s, stop_dt:%s, reason:%s' % \
                (self.class_name, '_get_ad_insights', ad_id, breakdown_attribute, start_dt, stop_dt, why))
            return False


    """
    按格式构建Breakdown Insights数据
    """
    def _build_breakdown_insights_data(self, ad_insights, account_category_info):
        try:
            insights_data_list = []
            if ad_insights:
                for ad_insight in ad_insights:
                    ...
                    for ad_parent_key in ad_parent_keys:
                        ad_parent_value = ad_insight[ad_parent_key]
                        ad_parent_value = ad_parent_value.replace(' ', '').replace('"', '').replace(',', '_')
                        ad_insights_data.append(ad_parent_value)

                    ad_breakdown_keys = ['buying_type', 'clicks', 'cpc', 'cpm', 'cpp', 'ctr', 'frequency', 'impressions']
                    for ad_breakdown_key in ad_breakdown_keys:
                        if ad_breakdown_key in ad_insight:
                            ad_insights_data.append(ad_insight[ad_breakdown_key])
                        else:
    ...
    def _build_collector_data(self, ad_insights, ad_creatives, account_category_info):
        try:
            ...
            if ad_insights and ad_creatives:
                ...
                insights_data	= self._build_breakdown_insights_data(ad_insights, account_category_info)
                date_data		= [self.start_dt, self.stop_dt, self.year, self.month, self.week, self.dt]
    ...
    def _save_collect_data(self, ads_info_list, account_category_info, file_path):
        try:
            ...
            header = ['company_id', 'business_id', 'account_id', 'account_name', 'campaign_id', 'campaign_name', 'adset_id', 'adset_name', 'ad_id', 'ad_name',
                    ...
                    'video_id', 'promoted_url', 'link', 'country', 'start_dt', 'stop_dt', 'year', 'month', 'week', 'dt'
            ]
            if ads_info_list:
                save_data_list = []
                for ad in ads_info_list:
                    ad_insights = self._get_ad_insights(ad["id"], 'country', self.start_dt, self.stop_dt)
