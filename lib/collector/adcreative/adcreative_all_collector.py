...
Created Time: 2017-06-23 19:27:02
"""
import os
import sys
import time
...
from adcreative_all_working import AdCreativeAllWorking


class AdCreativeAllCollector(BaseCollector):
    def __init__(self, conf, args):
        super(AdCreativeAllCollector, self).__init__(conf, args)
        self.company_id		= ''
        self.logger			= logging.getLogger('zeus_ads_all')
        self.wflogger		= logging.getLogger('zeus_ads_all_error')
            ...
            for access_token in access_token_list:
                ...
                if business_ids:
                    ...
                    for process_num in range(multiprocessing.cpu_count()):
                        account_working	= AdCreativeAllWorking(self, self.account_queue, ad_creative_path, access_token)
